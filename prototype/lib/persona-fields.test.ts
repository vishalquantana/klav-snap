...
test("insertTrait + listTraits: area/issueType/severity persisted and returned", async () => {
  ...
  const t2 = makeTrait({ id: "trait_pf_t2", text: "Navigation too slow" })
  await db.insertTrait(t2)

  const traits = await db.listTraits(SIM)
  const found1 = traits.find((t) => t.id === "trait_pf_t1")
  const found2 = traits.find((t) => t.id === "trait_pf_t2")
...
test("updateTrait: area/issueType/severity persisted and returned", async () => {
  ...
  const t = makeTrait({ id: "trait_pf_upd", text: "Layout breaks on mobile" })
  ...
  const traits = await db.listTraits(SIM)
  const found = traits.find((x) => x.id === "trait_pf_upd")
...
test("insertTraitEvent + listTraitEvents: area/issueType/severity persisted and returned", async () => {
  ...
  const t = makeTrait({ id: "trait_pf_evt1", text: "Performance issue in export" })
  await db.insertTrait(t)

  const e1 = makeEvent("trait_pf_evt1", {
    area: "export-modal",
    issueType: "performance",
    severity: "high",
    createdAt: NOW + 10,
  })
  const e2 = makeEvent("trait_pf_evt1", {
  ...
  const forTrait = events.filter((e) => e.traitId === "trait_pf_evt1")
...
test("listTraitEvents: traitId filter narrows to one trait's events", async () => {
  ...
  const tA = makeTrait({ id: "trait_pf_fA", text: "A pain" })
  const tB = makeTrait({ id: "trait_pf_fB", text: "B pain" })
  await db.insertTrait(tA)
  await db.insertTrait(tB)

  await db.insertTraitEvent(makeEvent("trait_pf_fA", { createdAt: NOW + 30, area: "area-A" }))
  await db.insertTraitEvent(makeEvent("trait_pf_fB", { createdAt: NOW + 40, area: "area-B" }))

  // Without filter: both traits' events present
  const all = await db.listTraitEvents(SIM)
  const hasA = all.some((e) => e.traitId === "trait_pf_fA")
  const hasB = all.some((e) => e.traitId === "trait_pf_fB")
  expect(hasA).toBe(true)
  expect(hasB).toBe(true)

  // With traitId filter: only A's events
  const filtered = await db.listTraitEvents(SIM, { traitId: "trait_pf_fA" })
  expect(filtered.every((e) => e.traitId === "trait_pf_fA")).toBe(true)
  expect(filtered.some((e) => e.traitId === "trait_pf_fB")).toBe(false)
...
test("getRecentlyResolvedTraits: returns contradicted/superseded only, newest-first, excludes active", async () => {
  ...
  const SIM2 = "sim_pf_resolved"
  const base = { simId: SIM2, projectId: PROJ }

  const tActive = makeTrait({ ...base, id: "trait_pf_r_active", text: "Active pain", status: "active", updatedAt: 3000 })
  const tContra = makeTrait({ ...base, id: "trait_pf_r_contra", text: "Contradicted pain", status: "contradicted", updatedAt: 2000, area: "billing", issueType: "label-copy" })
  const tSuper = makeTrait({ ...base, id: "trait_pf_r_super", text: "Superseded pain", status: "superseded", updatedAt: 1000, area: "onboarding", issueType: "flow" })
  ...
  // Must exclude active
  expect(resolved.some((t) => t.id === "trait_pf_r_active")).toBe(false)

  // Must include contradicted + superseded
  expect(resolved.some((t) => t.id === "trait_pf_r_contra")).toBe(true)
  expect(resolved.some((t) => t.id === "trait_pf_r_super")).toBe(true)

  // Newest-first by updated_at
  const ids = resolved.map((t) => t.id)
  const contraIdx = ids.indexOf("trait_pf_r_contra")
  const superIdx = ids.indexOf("trait_pf_r_super")
  expect(contraIdx).toBeLessThan(superIdx) // contradicted (updatedAt=2000) before superseded (1000)

  // Fields present
  const contra = resolved.find((t) => t.id === "trait_pf_r_contra")!
  ...
  expect(limited[0].id).toBe("trait_pf_r_contra") // newest
})
